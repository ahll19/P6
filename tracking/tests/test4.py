...
for j, track in enumerate(data):
    ...
    for i in range(3):
        ...
        ax[i].scatter(times[j], track[:, i], c=colors[j], zorder=1, label=f"True detections {j+1}", s=12, alpha=0.5)
        ax[i].plot(times[j], state[1:, i], c=c_colors[j], label=f"Track {j+1}", zorder=2, lw=0.8)
...
fig.tight_layout()
plt.savefig(f"test4_figs/tracks_{_xyz[i]}.pdf", bbox_inches="tight")
plt.show()
