...
t_orb,R_orb,A_orb,E_orb,dR_orb,_ = import_data(file_entire_orb[sat_])
...
for i in range(5):
    ...
    plt.tight_layout()
    #plt.savefig(fig_path+f"/track_false{i+1}_{file}.pdf")
    plt.show()
...
plt.legend(bbox_to_anchor=(1.04,1), loc="upper left", borderaxespad=0,fontsize=14)
plt.tight_layout()
plt.savefig(fig_path+f"/alltracks_false{i+1}_{file}.pdf")
plt.show()
'''
fig, axs = plt.subplots(2,2, sharex=True,sharey=False,figsize=(14,10))
fig.subplots_adjust(left=0.1, wspace=0.3)
